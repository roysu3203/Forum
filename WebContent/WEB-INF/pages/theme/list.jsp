...
<div class="row">
	<div class="span12">
		<a href="article.do?action=articleAddPage&forumsId=${forumsId}" 
			id="postArticle" class="btn btn-primary">發表文章</a>
		<br><br>
		<%-- 搜尋 --%>
		...
		<table class="table table-bordered">
			...
			<c:forEach var="theme" items="${themeList}">
				<tr>
					...
					<td>${fn:length(theme.articleList) - 1}</td>
					<td>${theme.hits}</td>
...
<div class="pagination">
	<ul>
		...
		<c:choose>
			...
			<c:otherwise>
				<li><a href="/Forums/theme.do?action=gotoArticleList&forumsId=${forumsId}&page=${locationPage - 1}">«</a></li>
			</c:otherwise>
		</c:choose>
		...
		<c:forEach var="i" begin="${beginPage}" end="${endPage}" step="1">
			...
			<c:choose>
				...
				<c:otherwise>
					<li><a href="/Forums/theme.do?action=gotoArticleList&forumsId=${forumsId}&page=${i}">${i}</a></li>
				</c:otherwise>
			</c:choose>
		</c:forEach>
	...
	<p class="text-right">討論區總共有${totalTheme}則討論主題。</p>
</div>
...
$(document).ready(function() {
	...
	$('#postArticle').click(function() {
		var isNotLogin = $('#logout').size() == 0; // 判斷是不是未登入
		if(isNotLogin) {
			alert("請先登入再發表文章。");
			return false;
		}
